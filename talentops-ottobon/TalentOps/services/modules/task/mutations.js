...
export const deleteTask = async (taskId) => {
    try {
        // 1. Delete associated RAG chunks (Cleanup vectors)
        const { error: chunkError } = await supabase
            .from('document_chunks')
            .delete()
            .eq('task_id', taskId);
        if (chunkError) console.error('Error deleting task chunks:', chunkError);

        // 2. Delete associated document metadata
        const { error: docError } = await supabase
            .from('documents')
            .delete()
            .eq('task_id', taskId);
        if (docError) console.error('Error deleting task documents:', docError);
        ...
        console.error('Failed to delete task cascadingly:', error);
        throw error;
    }
};
