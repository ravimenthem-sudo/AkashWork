    ...
    const handleAddTask = async (e) => {
        ...
        try {
            ...
            */
            ...
            const createdTasks = await taskService.createTask({
                newTask,
                user,
                orgId,
                effectiveProjectId,
            ...
            console.log('[DEBUG AddTask] Step 3 DONE: createdTasks =', JSON.stringify(createdTasks));

            // 4. Trigger RAG Ingestion for Guidance Files
            if (createdTasks && createdTasks.length > 0 && Object.keys(phaseFiles).length > 0) {
                const SERVER_URL = ''; // Use Vite proxy (same-origin) to avoid CORS blocks
                const ingestionPromises = [];

                createdTasks.forEach(task => {
                    Object.entries(phaseFiles).forEach(([phaseKey, file]) => {
                        const guidance = preparedValidations[phaseKey];
                        if (guidance && guidance.guidance_doc_url) {
                            ingestionPromises.push(
                                fetch(`${SERVER_URL}/api/chatbot/ingest`, {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({
                                        doc_id: `task_${task.id}_${phaseKey}`,
                                        org_id: orgId,
                                        project_id: effectiveProjectId,
                                        task_id: task.id,
                                        phase: phaseKey,
                                        file_url: guidance.guidance_doc_url,
                                        metadata: {
                                            title: `Task Guidance: ${newTask.title} (${phaseKey})`,
                                            task_id: task.id,
                                            phase: phaseKey,
                                            source: 'task_spec'
                                        }
                                    ...
                                    if (!data.success) throw new Error(data.message || 'Ingestion returned success false');
                                    console.log(`Successfully ingested ${phaseKey} for task ${task.id}`);
                                    addToast?.(`Guidance document for ${phaseKey} processed and ready!`, 'success');
                                }).catch(err => {
                                    console.error(`RAG Ingest failed for ${phaseKey}:`, err);
                                    addToast?.(`Failed to process guidance document for ${phaseKey}`, 'error');
                                })
                            );
                        }
                    });
                                                                ...
                                                                onChange={(e) => {
                                                                    ...
                                                                    if (file) {
                                                                        ...
                                                                        if (!validTypes.includes(file.type) && !file.name.match(/\.(pdf|doc|docx|txt)$/i)) {
                                                                            addToast?.('Only PDF, DOC, DOCX, and TXT files are supported for ingestion.', 'warning');
                                                                            return;
                                                                        }
                                                                        setPhaseFiles({ ...phaseFiles, [phase.key]: file });
                                                                    }
                                                                }}
                                                            />
