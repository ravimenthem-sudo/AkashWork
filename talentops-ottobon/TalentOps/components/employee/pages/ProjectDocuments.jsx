...
const ProjectDocuments = ({ userRole, addToast: parentAddToast = null }) => {
    ...
    const handleAddDocument = async () => {
        ...
        try {
            ...
            // Fetch org_id for RAG ingestion
            ...
            // Trigger RAG Ingestion on the backend
            try {
                const ingestResp = await fetch('/api/chatbot/ingest', {
                    method: 'POST',
                ...
                if (!ingestData.success) {
                    console.warn('RAG ingestion warning:', ingestData.message);
                    addToast('Document saved but AI indexing failed. The chatbot may not find this document.', 'warning');
                }
            } catch (ingestErr) {
                console.error('RAG Ingestion failing in background:', ingestErr);
                addToast('Document saved but AI indexing failed.', 'warning');
            }
