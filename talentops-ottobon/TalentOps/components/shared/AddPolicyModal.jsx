...
export const AddPolicyModal = ({ isOpen, onClose, onSuccess, orgId }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const createdPolicy = policyData[0];

            // 4. Trigger RAG Ingestion automatically
            try {
                // Use relative path or absolute if needed. The local server runs on port 8035 usually.
                // But the frontend usually proxies /api to the backend.
                await fetch('/api/chatbot/ingest', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        doc_id: createdPolicy.id,
                        org_id: orgId,
                        file_url: publicUrl,
                        metadata: {
                            title: formData.title,
                ...
                console.log('RAG Ingestion triggered for policy');
            } catch (ragErr) {
                console.error('Failed to trigger RAG ingestion:', ragErr);
                // We don't fail the whole operation if RAG fails, but we log it.
            }

            // 5. Send Notification to All Employees
